...
class TimeDisplayMode(StaticMode):
    ...
    def render(self, section, led_controller, data: Dict[str, Any]) -> None:
        ...
        if data.get("match_state", "unknown") in ["pre-match", "unknown"]:
            self._render_center(section, led_controller, abs(math.sin(time.time() * 2 * math.pi)) * section.length, self.active_color if (time.time() % 2.0 > 1) else self.inactive_color)
        elif direction == "left":
